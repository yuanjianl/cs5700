...
public class Experiment2 extends Experiment {
    ...
    public void start(String start_time, String end_time) {
        ...
        for (j = 0; j < CBR_RATE.length ; j++ ) {
            String shellScript = "/course/cs4700f12/ns-allinone-2.35/bin/ns experiment2.tcl " + TCP_KIND[firstTCP] + " " + TCP_KIND[secondTCP] + " " + CBR_RATE[j] + " " + start_time + " " + end_time;
            try {
        ...
        // Parse the results.
        for (j = 0; j < CBR_RATE.length ; j++ ) {
            readFile("my_experimental2_output_" + TCP_KIND[firstTCP] + "_" + TCP_KIND[secondTCP] + "_" + CBR_RATE[j] + "_" + start_time + ".tr");
        }
        ...
        Experiment.writeToFile("report/Report2_throughput" + "_" + TCP_KIND[firstTCP] + "_" + TCP_KIND[secondTCP] + "_" + start_time, throughputStr.toString());
        Experiment.writeToFile("report/Report2_delay" + "_" + TCP_KIND[firstTCP] + "_" + TCP_KIND[secondTCP] + "_" + start_time, delayStr.toString());
        Experiment.writeToFile("report/Report2_droprate" + "_" + TCP_KIND[firstTCP] + "_" + TCP_KIND[secondTCP] + "_" + start_time, dropRateStr.toString());
    }
